...
# aiogram 3.7+: use DefaultBotProperties (no parse_mode kwarg)
...
# ---------- Main ----------
async def main():
    # Resolve username first to avoid bad URL crashes
    global BOT_USERNAME
    try:
        me = await bot.get_me()
        if not BOT_USERNAME:
        ...
        digest = _exc_digest("get_me failed", e)
        logger.error(digest)
        await admin_notify("⚠️ " + digest)

    # DB with retry, never crash container on transient DB errors
    await db_init_with_retry()

    await admin_notify("✅ Bot started and DB is ready.")
